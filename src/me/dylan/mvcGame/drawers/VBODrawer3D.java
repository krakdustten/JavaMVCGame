...
public class VBODrawer3D {
    ...
    public static void draw3DCube(ArrayList<Float> array, int type, float x, float y, float z, float dx, float dy, float dz, float r, float g, float b, float a, float ty, float tx, float dty, float dtx){
        float[] f = new float[calcArraySizeForSquares(type, 1)];
        draw3DCube(f, 0, type, x, y, z, dx, dy, dz, r, g, b, a, ty, tx, dty, dtx);
        for(int i = 0; i < f.length; i++) array.add(f[i]);
    }

    public static int draw3DCube(float[] array, int offset, int type, float x, float y, float z, float dx, float dy, float dz, float r, float g, float b, float a, float tx, float ty, float dtx, float dty){
        switch (type){
            case COORDS_TYPE:
                addAll(array, offset,
                        x, y, 0.0f,
